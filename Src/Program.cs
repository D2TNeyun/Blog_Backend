...
using Src.Models;
    ...
    // Cấu hình Google Auth sau JwtBearer
    builder.Services.AddAuthentication(options =>
    {
    ...
    .AddCookie(options =>
    {
        options.Cookie.SameSite = SameSiteMode.None; // Quan trọng để cookie có thể chia sẻ
        options.Cookie.SecurePolicy = CookieSecurePolicy.Always; // Đảm bảo cookie chỉ được gửi qua HTTPS
    })
    .AddGoogle(options =>
    {
        var clientId = builder.Configuration.GetValue<string>("GoogleKeys:ClientId");
        var clientSecret = builder.Configuration.GetValue<string>("GoogleKeys:ClientSecret");

        if (string.IsNullOrEmpty(clientId) || string.IsNullOrEmpty(clientSecret))
        {
            throw new ArgumentException("Google ClientId or ClientSecret is not configured correctly.");
        }

        options.ClientId = clientId;
        options.ClientSecret = clientSecret;
        options.Scope.Add("email"); // Thêm quyền email để lấy thông tin email người dùng
        options.SaveTokens = true; // Lưu token nếu cần
    });
